...
- 前端生产构建通过；Rust 有 28 项本地自动化测试覆盖双存储、主机信任、凭据隔离、显式迁移、并发与故障恢复。
...
| S1 SSH 主机信任与凭据加固 | G2 | done | M1 | 主机指纹不再无条件接受，凭据不再依赖普通明文持久化 |
...
2. G2 已完成 `validated-local`：TOFU、fail-closed 变更、app-owned known_hosts、OS provider 边界和显式旧密码迁移均有本地证据。
3. 下一优先入口为 G3；G4 继续保留跨平台发布就绪，真实 provider/SSH/安装包证据仍未验证。
